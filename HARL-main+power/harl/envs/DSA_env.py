...
class DSA_Markov():
    ...
    def get_obs(self,action):
        obs2=self.sense()
        channel_state=obs2[0]
        obs_block = [channel_state[i:i + self.senselength] for i in range(self.n_channels)]
        obs22=[]
        for i in range(self.num_agents):
            obs22.append(obs_block[action[i]])
        agents = ['agent_0', 'agent_1', 'agent_2']
        obs = {agent: observation for agent, observation in zip(agents, obs22)}
    ...
    def step(self, action):
        ...
        obs = self.get_obs(action)#[array([0*, 0, 0, 1, 1, 1]), array([0, 1, 0, 0, 0, 0]), array([1, 0, 0, 1, 1, 1])]
