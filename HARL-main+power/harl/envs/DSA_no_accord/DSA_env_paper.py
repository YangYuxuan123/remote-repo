...
class DSA_Markov():
    def __init__(
        ...
        # 新增协议区域定义
        self.PU_CHANNELS = range(17, 32)  # 单独信道17-31
        self.ALOHA_CHANNELS = range(0, 11)  # 单独信道0-10
        self.TDMA_CHANNELS = range(11, 17)  # 单独信道11-16
    ...
    def check_overlap_and_ones(self,pos1, pos2, channel_state):
    ...
    def check_overlap_and_ones2(self,pos2, channel_state, channel_blockposition, action):
    ...
    def step(self, action):
        ...
        for k in range(self.num_agents):  # 0,1,2
            ...
            if (action[k] == self.n_channels):
                rewards[k] = -2
            else:
                for q in range(self.num_agents):
                    if action[q] != self.n_channels and self.check_overlap_and_ones(self.channel_position[action[k]], self.channel_position[action[q]],self.channel_state):  # 表示代理 q选择了一个有效的频道
                ...
                if self.channelagg_state[action[k]] == 1:
                    panduan,bili = self.check_overlap_and_ones2(self.channel_position[action[k]], self.channel_state, self.channel_position, action)
                    if (panduan):
                        #成功传输
                        ...
                        rewards[k] *= ((bili / self.num_agents) + (self.Dk - bili)) / self.Dk
    ...
    def render(self):
        ...
        for ch in range(len(self.channel_state)):
            if ch in self.PU_CHANNELS:
                ...
                if current_slot < TDMA_ACTIVE_SLOTS:
                    self.channel_state[ch] = 0  # 激活时隙，信道可用
                else:
                    self.channel_state[ch] = 1  # 非激活时隙，信道不可用
        ...
        self.obs_block = [self.channel_state[i:i + self.senselength] for i in range(self.n_channels)]
        self.channel_position = [list(range(i, i + self.senselength)) for i in range(self.n_channels)]
        self.channelagg_state = [
            1 if sum(1 for bit in observation if bit == 1) >= self.Dk else 0
            for observation in self.obs_block
        ]
