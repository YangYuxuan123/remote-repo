...
class DSA_Markov():
    ...
    def _build_location(self):
        ...
        # Initialize the location of PUs
        self.PU_TX_x = np.random.uniform(0, 20, self.n_channels + self.senselength - 1)
        self.PU_TX_y = np.random.uniform(0, 20, self.n_channels + self.senselength - 1)
        self.PU_RX_x = np.random.uniform(0, 20, self.n_channels + self.senselength - 1)
        self.PU_RX_y = np.random.uniform(0, 20, self.n_channels + self.senselength - 1)
        ...
        # Initialize the location of SUs transmitters
        self.SU_TX_x = np.random.uniform(0+4, 20-4, self.num_agents)
        self.SU_TX_y = np.random.uniform(0+4, 20-4, self.num_agents)
        # self.SU_TX_x = np.random.uniform(0+40, 20-40, self.n_su)
        # self.SU_TX_y = np.random.uniform(0+40, 20-40, self.n_su)

        # Initialize the distance between SUs' transmitter and receiver
        self.SU_d = np.random.uniform(2, 10, self.num_agents)
    ...
    def step(self, action):
        ...
        for k in range(self.num_agents):#0,1,2
            if (action[k] == self.n_channels) :#or self.channelagg_state[k]==0: # action is not choosing any channel
                ...
                if (self.channelagg_state[action[k]] == 1):
                    ...
                    if (check_overlap_and_ones2(self.channel_position[action[k]], self.channel_state, self.channel_position, action)):
        ...
        # Compute info
        info = {'agent_0': {}, 'agent_1': {}, 'agent_2': {},'agent_3': {}, 'agent_4': {}, 'agent_5': {}}
        done={'agent_0': False, 'agent_1': False, 'agent_2': False,'agent_3': False, 'agent_4': False, 'agent_5': False}
        obs = self.get_obs()#[array([0, 0, 0, 1, 1, 1]), array([0, 1, 0, 0, 0, 0]), array([1, 0, 0, 1, 1, 1])]
