...
class OffPolicyHARunner(OffPolicyBaseRunner):
    ...
    def train(self):
        """Train the model"""
        self.total_it += 1
        data = self.buffer.sample()##存储经验回放缓冲区，通过sample随机抽样一批数据，更新策略
        (
        ...
        #print(len(sp_share_obs),"jjjjjjjjjjjjj")
        # train critic
            ...
            if self.args["algo"] == "hasac":
                ...
                if self.args["algo"] == "had3qn":
                    ...
                    for agent_id in agent_order:
                        ...
                        #print(actions,"actionskkkkkk")
                        #print(actions,"actionskkkkkkkk")
                        #print(actions,"acnbnb")
                        actions = [action.requires_grad_(True) for action in actions]
                        actions_t = torch.cat(actions, dim=-1)
                        # print(actions_t)
                        # print(actions_t.requires_grad,"re_at")
                        # print(sp_share_obs,actions_t,"jjjjjjjjj")
                        # print(actions_t,"acddddd")
                        # print(sp_share_obs,"sp_share_obs")
                        # sp_share_obs = sp_share_obs.float()
                        # actions_t=actions_t.float()
                        value_pred = self.critic.get_values(sp_share_obs, actions_t)
