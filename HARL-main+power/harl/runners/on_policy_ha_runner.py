...
class OnPolicyHARunner(OnPolicyBaseRunner):
    ...
    def train(self):
        """Train the model."""
        actor_train_infos = []##初始化空列表，用于存储每个actor的训练信息

        # factor is used for considering updates made by previous agents
        #初始化一个全为1的数组，用于考虑由先前的agents做出的更新,记录前序智能体策略更新对当前智能体动作选择的影响权重
        ##对于HAPPO而言：
        factor = np.ones(
            (
        ...
        # compute advantages#计算优势，技术优势函数
        ...
        # normalize advantages for FP   #ep
        ...
        for agent_id in agent_order:
            ...
            # compute action log probs for the actor before update,旧策略动作概率计算.
