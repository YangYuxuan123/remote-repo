...
class OffPolicyBaseRunner:
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        ##创建用于渲染的环境：用于控制渲染的行为
        ##self.manual_render决定是否需要手动调用render函数进行渲染
        ##self.manual_expand_dims决定是否需要手动扩展并行环境的维度
        # env
    ...
    def run(self):
        ...
        if self.algo_args["render"]["use_render"]:  # render, not train
            self.render()
            return
        ##零数组，长度为并行环境的累积奖励，每个训练步骤中每个环境的奖励被添加到对应的self.train_episode_rewards元素中，当环境完成一个回合后对应self.train_episode_rewards元素被重置为0
        ...
        self.train_episode_failpu = np.zeros(
            self.algo_args["train"]["n_rollout_threads"]
        )
        self.train_episode_failtdma = np.zeros(
            self.algo_args["train"]["n_rollout_threads"]
        )
        self.train_episode_failaloha = np.zeros(
            self.algo_args["train"]["n_rollout_threads"]
        )
        self.train_episode_failsu = np.zeros(
            self.algo_args["train"]["n_rollout_threads"]
        )
        ...
        # print(steps,"tests")##50000
        ##计算训练过程中总步数，配置文件中设置的环境将运行的总步数；用于训练数据收集的并行环境数量
        ##Python中整除运算符，返回商的整数部分，结果是每个并行环境需要运行的步数  将结果赋值给steps，表示了训练过程中每个并行环境需要运行的步数
        update_num = int(  # update number per train
            self.algo_args["train"]["update_per_train"]
            * self.algo_args["train"]["train_interval"]
        )

        file_folder = './myresult/hasac/'
        ...
        for step in range(1, steps + 1):
            # print(obs,available_actions,"testtttt")##20
            actions = self.get_actions(
                obs, available_actions=available_actions, add_random=True
            )
            #print(actions,"acccccc") 10组actions
            (
                new_obs,
                new_share_obs,
                rewards,
            ...
            available_actions = new_available_actions # file_folder = '.\\myresult\\offpolicy\\'
            #print(share_obs[0])

            if step % self.algo_args["train"]["train_interval"] == 0:  #目前在收集数据，等step达到100的时候，更新一次模型参数
                if self.algo_args["train"]["use_linear_lr_decay"]:
                    if self.share_param:
                        self.actor[0].lr_decay(step, steps)
                    else:
                        #智能体行为策略：会有自己独立行为策略，通过为每个智能体创建独立actor实现的
                        ##虽然actor函数相同但是每个智能体行为策略可能不同，参数也不同
                        ##这段代码体现智能体异构性，即每个智能体可能有自己独立行为策略
                        for agent_id in range(self.num_agents):
            ...
            if step % self.algo_args["train"]["eval_interval"] == 0:   #目前在收集数据，等step达到200的时候，输出一次评估结果
                ...
                if self.algo_args["eval"]["use_eval"]:
                    # print("testtttttt",self.buffer.get_mean_success()) #
                    # print("testt3",self.buffer.get_mean_fail_PU_hist_1s()) #
                    # print("testt4",self.buffer.get_mean_fail_collision_hist_1s()) #
                    ...
                    print(
                        f"Average step reward in buffer: {self.buffer.get_mean_rewards()}."
                    )
                    print(
                        f"Average step success in buffer: {self.buffer.get_mean_success()}."
                    )
                    print(
                        f"Average step failpu in buffer: {self.buffer.get_mean_fail_PU_hist_1s()}."
                    )
                    print(
                        f"Average step failtdma in buffer: {self.buffer.get_mean_fail_TDMA_hist_1s()}."
                    )
                    print(
                        f"Average step failaloha in buffer: {self.buffer.get_mean_fail_ALOHA_hist_1s()}."
                    )
                    print(
                        f"Average step failsu in buffer: {self.buffer.get_mean_fail_collision_hist_1s()}."
                    )

                    reward_1_my.append(self.buffer.get_mean_rewards())
                    success_hist_1_my.append(self.buffer.get_mean_success())
                    fail_PU_hist_1_my.append(self.buffer.get_mean_fail_PU_hist_1s())
                    fail_TDMA_hist_1_my.append(self.buffer.get_mean_fail_TDMA_hist_1s())
                    fail_ALOHA_hist_1_my.append(self.buffer.get_mean_fail_ALOHA_hist_1s())
                    fail_collision_hist_1_my.append(self.buffer.get_mean_fail_collision_hist_1s())
                    ...
                    print(
                        f"Env {self.args['env']} Task {self.task_name} Algo {self.args['algo']} Exp {self.args['exp_name']} Step {cur_step} / {self.algo_args['train']['num_env_steps']}, average step success in buffer: {self.buffer.get_mean_success()/1000/20/self.num_agents}."
                    )
                    print(
                        f"Env {self.args['env']} Task {self.task_name} Algo {self.args['algo']} Exp {self.args['exp_name']} Step {cur_step} / {self.algo_args['train']['num_env_steps']}, average step failpu in buffer: {self.buffer.get_mean_fail_PU_hist_1s() / 1000 /20/ self.num_agents}."
                    )
                    print(
                        f"Env {self.args['env']} Task {self.task_name} Algo {self.args['algo']} Exp {self.args['exp_name']} Step {cur_step} / {self.algo_args['train']['num_env_steps']}, average step failpu in buffer: {self.buffer.get_mean_fail_TDMA_hist_1s() / 1000 / 20 / self.num_agents}."
                    )
                    print(
                        f"Env {self.args['env']} Task {self.task_name} Algo {self.args['algo']} Exp {self.args['exp_name']} Step {cur_step} / {self.algo_args['train']['num_env_steps']}, average step failpu in buffer: {self.buffer.get_mean_fail_ALOHA_hist_1s() / 1000 / 20 / self.num_agents}."
                    )
                    print(
                        f"Env {self.args['env']} Task {self.task_name} Algo {self.args['algo']} Exp {self.args['exp_name']} Step {cur_step} / {self.algo_args['train']['num_env_steps']}, average step failsu in buffer: {self.buffer.get_mean_fail_collision_hist_1s() / 1000/20 / self.num_agents}."
                    )
                ...
                self.save()
            np.save(file_folder + 'reward_7.npy', reward_1_my)  #这四行向后缩进了4格
    ...
    def warmup(self):
        ...
        ##预热阶段在正式开始训练前，先用随机动作填充经验回放缓冲区，过去经验中学习？训练开始前经验回放缓冲区是空的。立即开始训练会导致不稳定效果不好；预热阶段执行一些随机的动作，将这些动作和对应的观察、奖励等信息存储到经验回放缓冲区；为了给深度强化学习训练过程提供初始经验，帮助稳定并提高学习过程
        for _ in range(warmup_steps):
            # action: (n_threads, n_agents, dim)
            actions = self.sample_actions(available_actions)
            # print(actions,"acc8")#[[[ 1]
            #   [20]
            #   [ 2]]
            #
            #  [[12]
    ...
    def sample_actions(self, available_actions=None):
        ...
        for agent_id in range(self.num_agents):
            ...
            # print(action,"acc")
            # print(actions,"accs")
        ##预热阶段为每个智能体在每个并行环境中随机采样一个动作，无论动作空间是离散的还是连续的
        ##检查当前环境中智能体是否是离散，离散的actions数组形状从(n_agents, n_threads)转换为(n_threads, n_agents, 1)。
        #对离散动作空间的动作进行形状转换，许多情况下希望第一个维度是并行环境的数量，第二个维度是智能体数量
    ...
    def get_actions(self, obs, available_actions=None, add_random=True):
        ...
        if self.args["algo"] == "hasac":
            ...
            actions = []
            # print(obs)
            # print(available_actions,"hhhhhhhhhhh")  ##得到actions有问题
            ##获取智能体动作但使用场景和上下文不同
            ##通用方法，接受观察值、RNN状态、掩码和可用动作作为输入，调用actor方法计算动作、动作的对数概率和新的RNN状态，在任何需要获取智能体动作地方使用
            for agent_id in range(self.num_agents):
                actions.append(
    ...
    def eval(self, step):
        ...
        one_episode_rewards = []
        eval_episode_success_hist = []
        one_episode_success_hist = []
        eval_episode_fail_PU_hist = []
        one_episode_fail_PU_hist = []
        eval_episode_fail_TDMA_hist = []
        one_episode_fail_TDMA_hist = []
        eval_episode_fail_ALOHA_hist = []
        one_episode_fail_ALOHA_hist = []
        eval_episode_fail_collision_hist = []
        one_episode_fail_collision_hist = []

        file_folder_2 = './myresult/hasac/'
        fail_collisionmy = []
        success_histmy = []
        fail_PU_histmy = []
        # rewardmy = []
        for eval_i in range(self.algo_args["eval"]["n_eval_rollout_threads"]):
            ...
            one_episode_success_hist.append([])
            eval_episode_success_hist.append([])
            one_episode_fail_PU_hist.append([])
            eval_episode_fail_PU_hist.append([])
            one_episode_fail_TDMA_hist.append([])
            eval_episode_fail_TDMA_hist.append([])
            one_episode_fail_ALOHA_hist.append([])
            eval_episode_fail_ALOHA_hist.append([])
            one_episode_fail_collision_hist.append([])
            eval_episode_fail_collision_hist.append([])
        ...
        while True:
            ...
            for eval_i in range(self.algo_args["eval"]["n_eval_rollout_threads"]):
                ...
                one_episode_success_hist[eval_i].append(eval_success_hist_1[eval_i])
                one_episode_fail_PU_hist[eval_i].append(eval_fail_PU_hist_1[eval_i])
                one_episode_fail_TDMA_hist[eval_i].append(eval_fail_TDMA_hist_1[eval_i])
                one_episode_fail_ALOHA_hist[eval_i].append(eval_fail_ALOHA_hist_1[eval_i])
                one_episode_fail_collision_hist[eval_i].append(eval_fail_collision_hist_1[eval_i])
            ...
            for eval_i in range(self.algo_args["eval"]["n_eval_rollout_threads"]):
                if eval_dones_env[eval_i]:
                    ...
                    eval_episode_success_hist[eval_i].append(
                        np.sum(one_episode_success_hist[eval_i], axis=0)
                    )
                    one_episode_success_hist[eval_i] = []

                    eval_episode_fail_PU_hist[eval_i].append(
                        np.sum(one_episode_fail_PU_hist[eval_i], axis=0)
                    )
                    one_episode_fail_PU_hist[eval_i] = []

                    eval_episode_fail_TDMA_hist[eval_i].append(
                        np.sum(one_episode_fail_TDMA_hist[eval_i], axis=0)
                    )
                    one_episode_fail_TDMA_hist[eval_i] = []

                    eval_episode_fail_ALOHA_hist[eval_i].append(
                        np.sum(one_episode_fail_ALOHA_hist[eval_i], axis=0)
                    )
                    one_episode_fail_ALOHA_hist[eval_i] = []

                    eval_episode_fail_collision_hist[eval_i].append(
                        np.sum(one_episode_fail_collision_hist[eval_i], axis=0)
                    )
                    one_episode_fail_collision_hist[eval_i] = []
            ...
            if eval_episode >= self.algo_args["eval"]["eval_episodes"]:
                ...
                ) ##将列表中所有非空元素连接成一个Numpy数组
                eval_avg_rew = np.mean(eval_episode_rewards) #新奖励数组

                eval_episode_success_hist = np.concatenate(
                    [success for success in eval_episode_success_hist if success]
                )
                eval_avg_success = np.mean(eval_episode_success_hist)

                eval_episode_fail_PU_hist = np.concatenate(
                    [PU for PU in eval_episode_fail_PU_hist if PU]
                )
                eval_episode_fail_TDMA_hist = np.concatenate(
                    [TDMA for TDMA in eval_episode_fail_TDMA_hist if TDMA]
                )
                eval_episode_fail_ALOHA_hist = np.concatenate(
                    [ALOHA for ALOHA in eval_episode_fail_ALOHA_hist if ALOHA]
                )
                eval_avg_PU = np.mean(eval_episode_fail_PU_hist)

                eval_episode_fail_collision_hist = np.concatenate(
                    [failsu for failsu in eval_episode_fail_collision_hist if failsu]
                )
                eval_avg_failsu = np.mean(eval_episode_fail_collision_hist)
                ...
                if "smac" in self.args["env"]:
                    ...
                    print(
                        f"Eval average episode PU rate is {eval_avg_PU/3/5}, eval average episode length is {eval_avg_len}."
                    )
                    print(
                ...
                self.rewardmy.append(eval_avg_rew)
                    ...
                    # fail_PU_histmy.append(eval_avg_PU/3/5)

                # np.save(file_folder_2 + 'success_hist_5', success_histmy)
                # np.save(file_folder_2 + 'fail_collision_hist_5', fail_collisionmy)
                # np.save(file_folder_2 + 'fail_PU_hist_5', fail_PU_histmy)
                np.save(file_folder_2 + 'reward_episode_7.npy', self.rewardmy)

                    # print(
                    #     f"SUCCESS is {eval_success_hist_1}, failSU is {eval_fail_collision_hist_1}.\n"
                    # )
                    # print(
                    #     f"Eval average episode reward is {eval_avg_rew}, eval average episode length is {eval_avg_len}.\n"
                    # )
                ...
                if "smac" in self.args["env"]:
                    ...
                    )##将一些训练过程中关键信息写入日志文件；用于在每条日志信息后添加一个新行
                self.log_file.flush()
