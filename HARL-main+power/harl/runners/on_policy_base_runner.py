...
class OnPolicyBaseRunner:
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        #self.envs= DSA_MarkovEnv(env_args)
        ...
        if self.share_param:
            ...
            for agent_id in range(1, self.num_agents):
                assert (
                    ...
                    == self.envs.observation_space[0].shape
                ), "Agents have heterogeneous observation spaces, parameter sharing is not valid."
                assert (
                    self.envs.action_space[agent_id].shape == self.envs.action_space[0].shape
    ...
    def run(self):
        ...
        for episode in range(1, episodes + 1):
            ...
            for step in range(self.algo_args["train"]["episode_length"]):
                ...
                # success=0
                #self.envs = DSA_MarkovEnv(self.env_args)
                (
                    obs,
                    share_obs,
                    rewards,
    ...
    def warmup(self):
        ...
        for agent_id in range(self.num_agents):
            #print(obs,"ooo")
            #print(self.actor_buffer[agent_id].obs[0])
            #print(obs[:, agent_id].copy())
    ...
    def collect(self, step):
        ...
        rnn_state_collector = []
        for agent_id in range(self.num_agents):
            ##收集每个智能体动作和其他相关信息；需要为每个智能体分别计算动作，有一个循环遍历所有智能体，调用get_actions方法计算动作、动作的对数概率和新的RNN状态
        ...
        ##HADDPG类中get_actions方法使用e-greedy策略进行探索和利用平衡
