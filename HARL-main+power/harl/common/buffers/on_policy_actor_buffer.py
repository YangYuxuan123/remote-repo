...
class OnPolicyActorBuffer:
    ...
    def __init__(self, args, obs_space, act_space):
        ...
        ##四维Numpy数组用于存储RNN状态，数组每个维度含义
        #self.episode_length + 1,时间步数量
        #self.n_rollout_threads,并行环境数量
        # self.recurrent_n,RNN层数
        #self.rnn_hidden_size,RNN隐藏状态大小，即每个RNN单元的输出维度
        ...
        elif isinstance(act_space, np.ndarray):
            self.available_actions = np.ones(
                (self.episode_length + 1, self.n_rollout_threads, len(act_space)),
                dtype=np.float32,
            )
        # if act_space.__class__.__name__ == "Discrete":
        #     self.available_actions = np.ones(
